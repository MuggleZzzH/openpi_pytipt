...
# ==============================================================================
# Bash 脚本: LIBERO 90 评估实验 (本地环境版本)
#
# 功能:
#   - 仿照 libero_spatial.sh 案例，实现健壮的脚本执行
#   - 双重输出: 使用 'tee' 命令将所有输出同时打印到终端并写入日志文件。
#   - 日志管理: 自动在 'eval_logs/libero_90' 目录下创建带时间戳的日志文件。
...
# --- 脚本主体通过 tee 同时输出到终端和日志文件 ---
{
    ...
    # --- 步骤 5: 设置实验参数 ---
    echo "--- 步骤 5: 设置实验参数 ---"
    PROJECT_CATEGORY="ript-eval-base"           # 大类：评估实验
    BENCHMARK_TYPE="libero_90"             # 小类：90任务基准
    EXPERIMENT_BASE="eval_90"              # 实验基础名
