...
class PI0Policy(PreTrainedPolicy):
    ...
    def prepare_images(self, observation: dict[str, Tensor]):
        ...
        for key in present_img_keys:
            ...
            # 保证图像张量通道维位于第二维 (B, C, H, W)
            # 如果检测到形状为 (B, H, W, C)，则转置到期望形状。
            if img.ndim == 4 and img.shape[1] != 3 and img.shape[-1] == 3:
                img = img.permute(0, 3, 1, 2)

            # 若输入已是浮点且范围 [0,1]，先*255 → [0,255]
        ...
        # ========= 调试: 可选保存输入给模型的图像 =========
        if os.environ.get("DEBUG_SAVE_PROCESSED", "0") == "1":
            try:
                save_dir = "pi0/ript/debug_images/processed"
                os.makedirs(save_dir, exist_ok=True)
                
                # 调试输入图像的原始信息
                print(f"[DEBUG] 输入观测图像信息:")
                for key in present_img_keys:
                    orig_img = observation["image"][key]
                    print(f"  {key}: shape={orig_img.shape}, dtype={orig_img.dtype}, "
                          f"range=[{orig_img.min():.3f}, {orig_img.max():.3f}]")
                
                # 保存当前 batch 所有相机视图的首帧，便于比对
                cams = images.shape[1]
                ...
                # 只提示一次
                if not hasattr(self, "_debug_notice"):
                    print(f"[prepare_images] 已保存 processed 输入图像到 {save_dir}")
                    self._debug_notice = True
            except Exception as e:
                print(f"[prepare_images] 保存调试图像失败: {e}")
